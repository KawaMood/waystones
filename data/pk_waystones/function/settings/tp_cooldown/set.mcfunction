...
# Warn player
execute if score $pk.waystones.settings.tp_cooldown pk.value matches 1.. run tellraw @s [{text:"TP cooldown",color:"yellow"},{text:" has been set to ",color:"gray"},{nbt:"temp.cooldown",storage:"pk:common",interpret:true,color:"yellow"},{text:" tick(s)",color:"gray"}]
