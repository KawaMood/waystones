...
# @input
#   $mode (string): The distance calculation mode. Can be either "manhattan" (following grid) or "euclidean" (straight-line)
#   $value (int): Can be either:
#       - Distance in blocks per level consumption if the mode is "manhtattan" or "euclidean"
...
# Example: 
#   function pk_waystones:settings/xp_consumption/set {value:100,mode:"euclidean"} 

# Store parameters
$data modify storage pk:common temp.mode set value $(mode)
$data modify storage pk:common temp.value set value $(value)

# Control mode parameter value
execute unless data storage pk:common temp{mode:"fixed"} unless data storage pk:common temp{mode:"euclidean"} unless data storage pk:common temp{mode:"manhattan"} run return run tellraw @s {text:'Invalid "mode" argument: must be one of "fixed", "euclidean" or "manhattan".', color:"red"}

# Set default mode to euclidean (0)
