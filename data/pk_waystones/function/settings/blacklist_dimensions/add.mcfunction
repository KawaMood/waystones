...
execute unless score $already_contains pk.temp matches 1.. run tellraw @s [{text:"Added \"",color:"gray"},{nbt:"temp.dimension.id",storage:"pk:common",interpret:true,color:"yellow"},{text:"\" to the blacklisted dimensions",color:"gray"}]
