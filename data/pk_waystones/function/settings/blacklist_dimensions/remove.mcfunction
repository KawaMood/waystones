...
# If found
execute if score $found pk.temp matches 1.. run tellraw @s [{text:"Removed \"",color:"gray"},{nbt:"temp.dimension.id",storage:"pk:common",interpret:true,color:"yellow"},{text:"\" from the blacklisted dimensions",color:"gray"}]
